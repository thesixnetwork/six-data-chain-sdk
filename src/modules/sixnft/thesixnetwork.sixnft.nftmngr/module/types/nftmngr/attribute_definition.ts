...
export interface AttributeDefinitionV072 {
  name: string;
  data_type: string;
  required: boolean;
  display_value_field: string;
...
const baseAttributeDefinitionV072: object = {
  name: "",
  data_type: "",
  required: false,
  display_value_field: "",
...
export const AttributeDefinitionV072 = {
  encode(
    message: AttributeDefinitionV072,
    writer: Writer = Writer.create()
  ): Writer {
    if (message.name !== "") {
      writer.uint32(10).string(message.name);
  ...
  decode(input: Reader | Uint8Array, length?: number): AttributeDefinitionV072 {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = {
      ...baseAttributeDefinitionV072,
    } as AttributeDefinitionV072;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): AttributeDefinitionV072 {
    const message = {
      ...baseAttributeDefinitionV072,
    } as AttributeDefinitionV072;
    if (object.name !== undefined && object.name !== null) {
  ...
  toJSON(message: AttributeDefinitionV072): unknown {
    const obj: any = {};
    message.name !== undefined && (obj.name = message.name);
    message.data_type !== undefined && (obj.data_type = message.data_type);
    message.required !== undefined && (obj.required = message.required);
  ...
  fromPartial(
    object: DeepPartial<AttributeDefinitionV072>
